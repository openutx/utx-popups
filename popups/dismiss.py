...
def popup(devices=None):
    ...
    if UT.ENABLE:
        ...
        if UT.SYS:
            if UT.iOS:
                ...
                try:
                    images = sorted(
                        [
                            tpl
                            for tpl in os.listdir(images_path)
                            if str(tpl).endswith("png")
                        ]
                    )
                    ...
                    logger.warning(
                        f"Picture template path does not exist: {images_path}"
                    )
                    logger.error(f"{E}")
        elif UT.APP:
            if UT.TPL_DIR:
            ...
            try:
                images = sorted(
                    [tpl for tpl in os.listdir(images_path) if str(tpl).endswith("png")]
                )
                logger.info(f"Try to find template pictures: {images}")
                for step in range(UT.LOOP):
                    for img in images:
